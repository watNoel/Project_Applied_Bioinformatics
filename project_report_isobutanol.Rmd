...
We have to use samtools sort to sort the bam file before using picar tools, and then use picard tools with the option to remove the duplicates, using the option REMOVE_SEQUENSING_DUPLICATES(true). 
After that we can run cufflinks with -g .gff file and see the output. Then we need to figure out how to remove tRNA and rRNA. After that we proceed with Cuffmerge and Cuffdiff. 
